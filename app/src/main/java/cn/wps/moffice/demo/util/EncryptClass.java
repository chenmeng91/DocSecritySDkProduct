...
public class EncryptClass
{
	...
	public static int normalOpenFile(String path, OfficeOutputStream output) throws RemoteException
	{
		...
		{
			int count = 0;
			if (Util.isPDFFile(path))
			{
				path = "/sdcard/testInput.pdf";   //模拟设置打开文档使用其他文件流
			}
			else if (Util.isPptFile(path)) {
				path = "/sdcard/testInput.pptx";   //模拟设置打开文档使用其他文件流
			}
			else if (Util.isExcelFile(path)) {
				path = "/sdcard/export_log.et";   //模拟设置打开文档使用其他文件流
			}
			else
			{
				path = "/sdcard/aa.docx";   //模拟设置打开文档使用其他文件流
			}
